...
- Framework : `pytest`
- Emplacement : `tests/` (`test_config.py`, `test_data_loader.py`, `test_ui_sanity.py`, `test_file_preview_nav.py`, `test_search_index.py`, `test_search_ui.py`) — 96 tests
...
- `VERSION` : fichier unique définissant la version officielle (`2.0`).
...
- `data_loader.py` :
  ...
  - **[v2.0] Export Markdown** : `build_conversation_markdown(conv_id, title, project, images=None)` — en-tête (titre/projet/date/ID/date d'export) + messages (`### 👤 Utilisateur` / `### ✨ Antigravity`) + annexe `## Annexe — Artéfacts de session` (walkthrough.md / implementation_plan.md / task.md) + section `## Images`. `_collect_session_images(brain)` ramasse les images dans `brain/<id>/` (générées), `.tempmediaStorage/` (temporaires) et `.user_uploaded/` (fournies par l'utilisateur). `_copy_session_images(brain, dest)` les copie dans `<nom_du_md>_images/` à côté du `.md` (collisions de noms désambiguïsées par suffixe) et renvoie des chemins relatifs `![nom](<dir>/<nom>)` → le `.md` exporté est **autonome** (on voit l'image de référence ET le rendu). `_write_export()` orchestre copie + génération ; `export_conversation_to_project()` → `<racine>/<projet>/_conversations/<date>_<slug>_<id8>.md`, `export_conversation_to_path()` → emplacement libre. Message de retour : `<chemin> (+N images)`.
...
- `antigravity_manager.py` : interface graphique moderne **PyQt6** (v2.0) :
