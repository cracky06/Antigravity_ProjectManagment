...
def _codex_conversation_body_html(conv) -> tuple[str, str]:
    """Retourne (titre_affiché, html_de_la_section) pour une CodexConv."""
    from codex_loader import load_codex_messages
    from data_loader import _sanitize_message_text
    ...
    if messages:
        for msg in messages:
            ...
            cls = "msg-user" if is_user else "msg-model"
            who = "user" if is_user else "model"
            name = "👤 Utilisateur" if is_user else "◉ Codex"
            ts = msg.get("timestamp", "")
            body = _md_to_html(
                _sanitize_message_text((msg.get("text", "") or "").rstrip(), conv.project_root)
            )
...
def _build_codex_full_html(project_name: str, convs, export_date: str) -> tuple[str, int]:
    """Équivalent de `_build_full_html` pour la source Codex — pas
    d'annexe d'images (aucune à corréler pour cette source)."""
    conv_list = list(convs)
    ...
    sections: list[str] = []
    for c in conv_list:
        _disp, body_html = _codex_conversation_body_html(c)
        sections.append(body_html)
...
def export_codex_project_to_pdf(project_name: str, convs, pdf_path: str | Path) -> tuple[bool, str]:
    """Génère le PDF de toutes les conversations Codex `convs` d'un
    projet (même moteur Edge/Chromium headless que `export_project_to_pdf`).
    ...
    try:
        ...
        pdf_path.parent.mkdir(parents=True, exist_ok=True)
        export_date = datetime.now().strftime("%Y-%m-%d %H:%M")
        html, _n = _build_codex_full_html(project_name, convs, export_date)
        ...
        logger.debug("PDF projet Codex %s -> %s (html/%s)", project_name, pdf_path, Path(browser).name)
        return True, str(pdf_path)
    except Exception as exc:
        logger.warning("Échec export PDF Codex %s : %s", project_name, exc)
        return False, f"Échec de l'export PDF : {exc}"
