...
- **Glisser-déposer (Drag & Drop) de conversations** : déplacement intuitif d'une discussion Antigravity par simple glisser-déposer sur un dossier projet cible dans l'arbre (avec boîte de confirmation de sécurité) *(v2.12)*
- **Préservation de l'état de l'arbre au rechargement (refresh)** : mémorisation et réouverture automatique de tous les dossiers projets qui étaient dépliés avant l'actualisation, dépliage automatique du projet parent et repositionnement direct sur la conversation active (sélection et défilement centré) *(v2.12)*
- **Filtrage intelligent des projets Antigravity** : masquage de tous les dossiers de développement génériques ne contenant aucun marqueur Antigravity (.agent/.gemini/.antigravityrules/agy.json) ni configuration officielle (.gemini/config/projects/*.json) ni conversation active, supprimant la pollution visuelle de l'arborescence *(v2.11)*
- **Dialogue post-déplacement assoupli** : boîte de dialogue proposant deux boutons « Fermer » et « Redémarrer Desktop » avec une note suggérant le redémarrage uniquement si Desktop ne s'est pas actualisé en direct *(v2.11)*
- **Nettoyage complet lors de la suppression de discussion** : purge conjointe des fichiers locaux et de la table conversation_summaries.db garantissant la disparition immédiate côté Antigravity Desktop et IDE *(v2.11)*
- **Stabilité et non-écrasement de la barre latérale des projets** : verrouillage de la largeur minimale de la sidebar à 220px (`setCollapsible(False)`, stretch factors), et retour à la ligne automatique (`setWordWrap(True)`) sur les libellés de titre et métadonnées du chat pour empêcher qu'un titre long n'impose une largeur gigantesque et n'écrase le panneau des projets *(v2.10)*
- **Icône officielle Codex** : intégration de l'asset `assets/codex.png` dans le sélecteur de sources et sur chaque discussion dans l'arbre Codex *(v2.10)*
- **Correction des icônes Antigravity Desktop vs IDE** : affectation exacte de l'icône sur fond blanc (`antigravity_white.png`) pour Antigravity Desktop et fond sombre (`antigravity_black.png`) pour Antigravity IDE *(v2.10)*
- **Redémarrage contrôlé de Google Antigravity Desktop** : boîte de dialogue explicite avec bouton unique « Redémarrer Desktop » après un déplacement de conversation *(v2.10)*
- **Synchronisation complète du déplacement de conversation avec Google Antigravity Desktop** : mise à jour conjointe de `conversation_summaries.db` (`project_id`, `workspace_uris`, `raw_summary`), du champ 4 d'`agyhub_summaries_proto.pb` et de `trajectory_metadata_blob` dans `conversations/<cid>.db` avec **redémarrage automatique d'Antigravity Desktop** s'il est en cours d'exécution pour recharger immédiatement la discussion sous son nouveau projet *(v2.9)*
